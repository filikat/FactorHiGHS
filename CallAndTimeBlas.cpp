...
#ifdef FINEST_TIMING
  DC.sumTime(kTimeDenseFact_syrk, GetTime() - t0);
#endif
}
...
void callAndTime_dgemm(char transa, char transb, int m, int n, int k,
  ...
  dgemm_(&transa, &transb, &m, &n, &k, &alpha, A, &lda, B, &ldb, &beta, C,
         &ldc);
#ifdef FINEST_TIMING
  DC.sumTime(kTimeDenseFact_gemm, GetTime() - t0);
#endif
}
...
#ifdef FINEST_TIMING
  DC.sumTime(kTimeDenseFact_trsm, GetTime() - t0);
#endif
}
...
#ifdef FINEST_TIMING
  DC.sumTime(kTimeDenseFact_gemv, GetTime() - t0);
#endif
}
...
#ifdef FINEST_TIMING
  DC.sumTime(kTimeDenseFact_copy, GetTime() - t0);
#endif
}
...
#ifdef FINEST_TIMING
  DC.sumTime(kTimeDenseFact_axpy, GetTime() - t0);
#endif
}
...
void callAndTime_dscal(int n, const double da, double* dx, int incx,
  ...
  dscal_(&n, &da, dx, &incx);
#ifdef FINEST_TIMING
  DC.sumTime(kTimeDenseFact_scal, GetTime() - t0);
#endif
}
